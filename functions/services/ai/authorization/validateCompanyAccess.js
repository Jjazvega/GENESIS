...
async function validateCompanyAccess({ user, companyId }) {
  ...
  if (!companySnap.exists) fail(403, 'Empresa no válida o sin acceso.', 'COMPANY_ACCESS_DENIED');
  const company = companySnap.data() || {};
  if (!isActiveStatus(company.status)) fail(403, 'La empresa no está activa para usar IA.', 'COMPANY_ACCESS_DENIED');
  ...
  if (!membershipSnap.exists) fail(403, 'Se requiere membresía activa en la empresa para usar IA.', 'MEMBERSHIP_INACTIVE');
  const membership = membershipSnap.data() || {};
  if (membership.companyId !== companyId || membership.userUid !== user.uid || !isActiveStatus(membership.status)) fail(403, 'Se requiere membresía activa en la empresa para usar IA.', 'MEMBERSHIP_INACTIVE');
  ...
  if (!AI_ALLOWED_ROLES.has(role)) fail(403, 'Tu rol no permite usar IA en esta empresa.', 'COMPANY_ACCESS_DENIED');
